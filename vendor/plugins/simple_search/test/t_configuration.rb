...
class PluginSetupTest < Test::Unit::TestCase
  ...
  def test_condition
    assert !ActiveSearch::indexers(Page)[2].will_consume_record?(@page), "This page is not approved?"
    assert ActiveSearch::indexers(Page)[2].will_consume_record?(@second_page), "This page is approved?"
  end
  ...
  def test_configuration_options  
    ...
    assert_equal :an_index, Page.indexers[0].index_column_name
      "an_index should be setup as index column"

    assert Page.indexers[0].respond_to?(:index_column_name)
      "The indexer should be able to report the index column name"
    ...
    assert_equal [:first_name, :last_name, :birth_date], PageAuthor.indexers[0].indexed_columns,
